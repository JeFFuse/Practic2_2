...
Node* createList() {
	...
	cout << "Затраченое время: " << duration << " ns" << endl;
	return head;
}
...
int main() {
	...
	while (true) {
		...
		cout << "4. Обмен элемента " << endl;
		cout << "5. Получить элемент " << endl;
		cout << "0. Выход " << endl;
		cin >> task;
		cin.ignore();
		switch (task) {
		case 1:
			while (true) {
				...
				cout << "1. Рандом " << endl;
				cout << "2. Пользователь вводит число " << endl;
		...
		case 5:
			...
			if (task5 == 1) {
				...
				if (foundNode != NULL) {
					cout << "Число: " << foundNode->data << endl;
				}
			}
			else if (task5 == 2) {
				int value;
